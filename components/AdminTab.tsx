...
const AdminTab: React.FC<{ currentUser: UserProgress }> = () => {
  ...
  const deleteUser = (id: string) => {
    if (confirm("REMOVER USUÁRIO DEFINITIVAMENTE?")) {
      const db = JSON.parse(localStorage.getItem('lingualeap_db') || '[]');
    ...
    <div className="space-y-8 animate-fadeIn pb-20">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Sincronização Total - Todos os Hubs de Professores</p>
        </div>
        <button onClick={() => { setShowForm('add'); setFormData({username:'',password:'',role:'student',level:'A1',email:'',whatsapp:'',teacherId:''}); }} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-100">Registrar User</button>
      </header>
        ...
        <table className="w-full text-left whitespace-nowrap">
          <thead className="bg-slate-50/50">
            <tr>
              ...
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Hub Responsável</th>
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Sessão</th>
              ...
              <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-8 py-6">
                  <span className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200">
                    {u.teacherId ? users.find(t=>t.id===u.teacherId)?.username.toUpperCase() : 'ADMIN CORE'}
                  </span>
                </td>
                <td className="px-8 py-6">
                  <div className="flex flex-col gap-0.5 text-[10px] font-bold text-slate-500">
                    ...
                    <span>TOTAL: {formatTime(u.totalSessionTime)}</span>
                  </div>
                </td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl relative">
            <h3 className="text-2xl font-black mb-6 uppercase">Gestão de Usuário</h3>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <button className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase">Salvar Dados</button>
              <button type="button" onClick={()=>setShowForm(null)} className="w-full text-slate-400 font-bold text-xs uppercase mt-2">Fechar</button>
            </form>
          </div>
        </div>
      )}
