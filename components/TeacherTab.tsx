...
const TeacherTab: React.FC<{ currentUser: UserProgress }> = ({ currentUser }) => {
  ...
  const [formData, setFormData] = useState({
    username: '', password: '', level: 'A1' as ProficiencyLevel,
    email: '', whatsapp: ''
  });
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (showForm === 'add') {
      const newUser: UserProgress = {
        ...
        email: formData.email, whatsapp: formData.whatsapp,
        teacherId: currentUser.id, xp: 0, streak: 0, completedLessons: [], certificates: [],
    ...
    <div className="space-y-8 animate-fadeIn pb-20">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Alunos em seu círculo: {myStudents.length}</p>
        </div>
        <button onClick={() => { setShowForm('add'); setFormData({username:'',password:'',level:'A1',email:'',whatsapp:''}); }} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-100">Registrar Aluno</button>
      </header>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl relative">
            <h3 className="text-2xl font-black mb-6 uppercase">Perfil Aluno</h3>
            <form onSubmit={handleSave} className="space-y-4">
              <input required value={formData.username} onChange={e=>setFormData({...formData, username:e.target.value})} placeholder="NOME" className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-bold outline-none focus:border-indigo-600 uppercase" />
